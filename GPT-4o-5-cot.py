...
if __name__ == "__main__":
    ...
    for i, data_set in enumerate(data_list):
        data_set_answer = []
        target_file = f'./demo/5-shot-cot-{file_list[i]}.json'

        if file_list[i] == 'GAOKAO':
            example = []
            for question, answer in examples['gaokao'][0:5]:
                example.append(f'Problem: {question}\nAnswer: {answer}\n\n')
        elif file_list[i] == 'SAT':
            example = []
            for question, answer in examples['sat'][0:5]:
                example.append(f'Problem: {question}\nAnswer: {answer}\n\n')
        else:
            example = []
        ...
        for line in tqdm(data_set):
            sys_prompt = ('You are a math expert.\n'
                          'You need to learn examples and answer the final question.\n'
